...
int __forceinline BeginContext(game_state &state, game_var_space *vars, bool isUIContext) {
...
void __forceinline EndContext(game_state &state, int handle) {
	//Doesn't happen...
        ...
        switch (rec) {
            case executionType::Instruction: {
                ...
                switch (typeHash) {
                    ...
                    default: instrResult = instr->exec(state, vm);
                }
                //instr->exec(state, vm)
                if (instrResult) {
                    // end evaluation
